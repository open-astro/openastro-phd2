...
- New `codeql.yml`: CodeQL semantic security analysis (security-extended query suite) of the C++ tree — post-merge on `main`, weekly, and on demand; findings surface in the Security tab. The PR clang-tidy job additionally runs the Clang Static Analyzer (`clang-analyzer-*`) on changed lines.
